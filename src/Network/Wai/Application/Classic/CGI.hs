...
cgiGitBackendApp body req respond = E.bracket setup teardown (respond <=< cgi)
  where
...
execGitBackendProcess req = do
 ...
 where
    proSpec naddr epath = CreateProcess {
      ...
      , cwd = Nothing
      , env = Just $ makeEnv req naddr "git http-backend" (show $ rawPathInfo req) "git http-backend" epath
      , std_in = CreatePipe
      , std_out = CreatePipe
      , std_err = Inherit
      , close_fds = True
...
makeEnv req naddr scriptName pathinfo sname epath = addPath epath . addLen . addType . addCookie $ baseEnv
  where
    tp = case pathInfo req of
            [] -> error "Should never happen." -- FIXME
            (_:rest) -> "/tmp/shared-wolf/accounts/379cf27b-84c1-44bd-a73f-1f4315f5be9c/data/.git/" ++ T.unpack (T.intercalate "/" rest)
