...
module Network.Wai.Application.Classic.Lang
    ( parseLang
    ) where

import Control.Applicative hiding (optional)
import Data.Attoparsec.ByteString (Parser, parseOnly, takeWhile)
import Data.Attoparsec.ByteString.Char8
       (char, count, digit, option, sepBy1, space, string)
import Data.ByteString.Char8
       hiding (count, map, notElem, take, takeWhile)
...
parseLang bs =
    case parseOnly acceptLanguage bs of
...
qvalue :: Parser Int
qvalue =
    1000 <$ (char '1' *> optional (char '.' *> range 0 3 digit)) <|>
